...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let stars = [];
    const DOM = {};
    const DYNAMIC_DOM = { buildings: {}, upgrades: {}, prestigeUpgrades: {} };
    let starfieldCanvas, starfieldCtx, nebulaCanvas, nebulaCtx;
    ...
    const defaultGameState = (isPrestige = false) => {
        ...
        if (isPrestige) {
            ...
            synergy_global",source:"ai",multiplier:1.1},'global_1':{name:"Compressão Global",description:"Dobra (2x) toda a sua geração de Data (Cliques e DPS).",cost:5e4,requirement:e=>e.buildings.quantum.count>=1,type:"global_multiplier",multiplier:2},'unlock_abilities':{name:"Desbloquear Processador de Efeitos",description:"Habilita as habilidades ativas Overclock e Data Dump.",cost:25e3,requirement:e=>e.buildings.quantum.count>=1,type:"unlock",target:"abilities"}};
    ...
    // --- INICIALIZAÇÃO E LÓGICA DE START ---
    function init(){
        const SELECTORS = ['background-effects','data-count','kernel-cores-count','prestige-bonus','auto-clicks-per-sec','dps-count','dpc-count','data-orb','buildings-container','upgrades-container','prestige-upnewState.stats.lastPrestigePlayTime = oldState.stats.playTime;
    ...
    // --- GAME STATE ---
    const defaultGameState = (isShipLaunch() { if(gameState.shipLaunched) return; const canLaunch = BUILDINGS_DATA.every(b => b.id === 'script' || gameState.buildings[b.id].count >= 250); if(canLaunch){ gameState.shipLaunched = true; DOM['ship-launch-overlay'].classList.add('active'); DOM['odyssey-svg'].classList.add('is-launched'); checkAchievements(); } }
    ...
    function updateUI(force = false){
        ...
        if(DOM.prestigeUpgradesHeader) DOM.prestigeUpgradesHeader.style.display = showPrestigeHeader ? 'block' : 'none';
    ...
    async function updateModal(activeTab) {
        ...
        if (activeTab === 'hall-of-fame-contentPrestige = false) => {
        const oldState = gameState;
        const newState = {
            data: 0,
            devMode: false,
    ...
    function updateUI(force = false){
        ...
        const showPrestigeHeader = gameState.stats.prestigeCount > 0 && Object.keys(PRESTIGE_UPGRADES_DATA).some(id => !gameState.prestigeUpgrades.includes(id) && gameState.stats.prestigeCount >= PRESTIGE_UPGRADES_DATA[id].prestigeReq);
        DOM.prestigeUpgradesHeader.style.display = showPre') {
            let list = '<ul>';
    ...
    function getSaveKey() { const player = AuthManager.getLoggedInUser(); return player ? `quantumOdysseySave_v5.3_${player}` : null; }
