...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const starfieldCanvas = document.getElementById('starfield-canvas'),starfieldCtx = starfieldCanvas.getContext('2d'),nebulaCanvas = document.getElementById('nebula-canvas'),nebulaCtx = nebulaCanvas.getContext('2d');
    let stars = [];
    ...
    function showToast(e, duration = 4900){const t=document.createElement("div");t.className="toast",t.textContent=e,DOM["toast-container"].appendChild(t),setTimeout(()=>t.remove(),duration)};
    ...
    function updateGameTheme(){const e=document.body.classList,t=gameState.stats.prestigeCount;e.remove("theme-cosmic","theme-singularity","theme-supernova","theme-whitedwarf");if(t>=15)e.add("theme-whitedwarf");else if(t>=7)e.add("theme-supernova");else if(t>=3)e.add("theme-singularity");else if(t>=1)e.add("theme-cosmic")};
    function updateShipVisuals(){const e=gameState.buildings;const t=(t,n)=>{if(!t)return;let s=0;n>=1000?s=5:n>=250?s=4:n>=50?s=3:n>=10?s=2:n>=1&&(s=1),t.classList.remove("level-1","level-2","level-3","level-4","level-5"),s>0&&t.classList.add(`level-${s}`)};const n=[{id:"ship-hull",building:e.script},{id:"ship-engine",building:e.cpu},{id:"ship-bridge",building:e.server},{id:"ship-wings",building:e.quantum},{id:"ship-sensors",building:e.ai},{id:"ship-plating",building:e.dyson}];let s=!0;n.forEach(e=>{const n=document.getElementById(e.id);n?.classList.toggle("active",e.building.count>0),t(n,e.building.count);if(e.id !== 'ship-hull' && e.building.count<250)s=!1});document.getElementById("engine-glow-group")?.classList.toggle("active",e.cpu.count>=1);DOM["odyssey-svg"].classList.toggle("is-active",s&&!gameState.shipLaunched)};
    
    let lastUIUpdate=0;
    function updateUI(){
        const t=performance.now();
        if(t-lastUIUpdate<100&&document.activeElement.tagName!=="INPUT")return; lastUIUpdate=t;
        ...
        DOM["prestige-progress-text"].textContent = `${formatNumber(gameState.stats.totalDataThisPrestige)} / ${formatNumber(nextCost)}`;

        DOM["dev-panel"].style.display=gameState.devMode?"block":"none";
        
        DOM["buildings-container"].innerHTML = '';
        BUILDINGS_DATA.forEach(b => {
            const state = gameState.buildings[b.id], el = document.createElement("div"), cost = calculateCost(b.baseCost, state.count), isMax = state.count >= 1000;
            el.className = "item"; el.onclick = (e) => buyBuilding(e, b.id);
            el.innerHTML = `<div class="item-info"><h3>${b.name}</h3><p>${b.description}</p><p>Custo: <span class="cost">${isMax ? 'MAX' : formatNumber(cost)}</span></p></div><span class="item-count">${state.count}</span>`;
            el.classList.toggle("disabled", gameState.data < cost || isMax);
            DOM["buildings-container"].appendChild(el);
        });

        DOM["upgrades-container"].innerHTML = '';
        Object.keys(UPGRADES_DATA).forEach(id=>{const up=UPGRADES_DATA[id];if(gameState.upgrades.includes(id)||!up.requirement(gameState))return;const el=document.createElement("div");el.className="item";el.onclick=()=>buyUpgrade(id);el.innerHTML=`<div class="item-info"><h3>${up.name}</h3><p>Custo: ${formatNumber(up.cost)}</p><small>${up.description}</small></div>`;el.classList.toggle("disabled",gameState.data<up.cost);DOM["upgrades-container"].appendChild(el)});
        
        DOM["prestige-upgrades-container"].innerHTML = '';
        if (gameState.stats.prestigeCount > 0) {
            const header = document.createElement('h3'); header.className = 'sub-header'; header.textContent = 'Upgrades de Kernel';
            DOM["prestige-upgrades-container"].appendChild(header);
            const prestigeList = document.createElement('div'); prestigeList.className = 'item-list';
            DOM["prestige-upgrades-container"].appendChild(prestigeList);
            Object.keys(PRESTIGE_UPGRADES_DATA).forEach(id=>{const up=PRESTIGE_UPGRADES_DATA[id];if(gameState.prestigeUpgrades.includes(id)||gameState.stats.prestigeCount<up.prestigeReq)return;const el=document.createElement("div");el.className="item prestige-upgrade";el.onclick=()=>buyPrestigeUpgrade(id);el.innerHTML=`<div class="item-info"><h3>${up.name}</h3><p>Custo: ${formatNumber(up.cost,0)} Núcleos</p><small>${up.description}</small></div>`;el.classList.toggle("disabled",gameState.prestige.cores<up.cost);prestigeList.appendChild(el)});
        }
    ...
    async function updateModal(activeTab) {
        ...
        if (activeTab === 'ranking-content') {
            ...
            let table = `<table class="ranking-table"><tr><th>#</th><th>Hacker</th><th>Score</th></tr>`;
    ...
    function addEventListeners() {
        ...
        DOM["dev-add-data"].addEventListener("click",()=>{gameState.data+=1e8; updateUI();});
        DOM["dev-add-kernels"].addEventListener("click",()=>{gameState.prestige.cores+=100; updateUI();});
        DOM["dev-reset-cooldowns"].addEventListener("click",()=>{gameState.activeAbilities.overclock.cooldown_until=0;gameState.activeAbilities.datadump.cooldown_until=0; updateUI();});
    }
